...
function parseEntryFile(raw) {
  let metaObj = {};
  let body = raw;
  const fm = FRONTMATTER_RE.exec(raw);
  if (fm && fm[1]) {
    try {
      metaObj = yaml.load(fm[1]) ?? {};
    } catch {
      metaObj = {};
    }
...
function serializeEntryFile(entry) {
...
function normalizeMeta(obj) {
  ...
  const date = typeof obj.date === "string" ? obj.date : formatKeyFallback();
  const title = typeof obj.title === "string" ? obj.title : "\u672A\u547D\u540D";
  const mood = MOODS.includes(obj.mood) ? obj.mood : "neutral";
  const weather = WEATHERS.includes(obj.weather) ? obj.weather : "unknown";
  const location = typeof obj.location === "string" ? obj.location : void 0;
  const tags = Array.isArray(obj.tags) ? obj.tags.filter((t) => typeof t === "string") : [];
  const assets = Array.isArray(obj.assets) ? obj.assets.filter(isAssetRef) : [];
  const nowIso = (/* @__PURE__ */ new Date()).toISOString();
  const created_at = typeof obj.created_at === "string" ? obj.created_at : nowIso;
  const updated_at = typeof obj.updated_at === "string" ? obj.updated_at : nowIso;
  return {
    id,
    date,
    title,
...
function isAssetRef(v) {
  ...
  const o = v;
  return typeof o.path === "string" && (o.kind === "image" || o.kind === "audio" || o.kind === "video" || o.kind === "attachment");
}
...
// src/lib/vault.ts
function entryFilePath(dateKey) {
  const [y, m] = dateKey.split("-");
  return `entries/${y}/${m}/${dateKey}.md`;
}
function versionDir(dateKey) {
  return `versions/${dateKey}`;
}
function versionFilePath(dateKey, version) {
  return `${versionDir(dateKey)}/v${version}.md`;
}

// src/lib/version.ts
async function listVersions(storage, dateKey) {
  ...
  for (const f of files) {
    ...
    try {
      ...
      versions.push({
        ...
        preview: body.replace(/[#>*_`~]/g, " ").replace(/\s+/g, " ").trim().slice(0, 80)
      });
    } catch {
    }
  }
...
var MemStorage = class {
  ...
  async readText(p) {
    ...
    if (f.text !== void 0) return f.text;
  ...
  async readBytes(p) {
    ...
    if (f?.text !== void 0) return new TextEncoder().encode(f.text);
    throw new Error("missing bytes " + p);
  }
  async writeBytes(p, d) {
...
function makeEntry(date, title, body) {
