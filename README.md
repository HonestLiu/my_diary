...
3. 对比后上传 / 下载差异文件
4. 两侧自上次同步以来都被修改 → 写入 `conflicts/` 冲突副本，**绝不静默覆盖**，由用户在设置中手动解决（保留本地 / 采用远端）

健壮性设计（解决此前「动不动就冲突」的根因，两端协议一致）：

- **远端清单只反映桶内真实对象**：每次同步后按「桶内对象 + 本次确认结果」重建清单，绝不把单台设备的本地视图原样写回 —— 否则两台设备会互相踩踏清单，产生虚假冲突。
- **首次同步不判冲突**：没有共同基线就谈不上「两侧各自变更」。以远端为准采纳，本地不同版本保留为 `conflicts/<name>.replaced.md` 安全副本，不再一上来就批量报冲突。
- **只推进已成功对账的路径**：某文件同步失败时保留其旧基线，下次同步自动重试；清单推送失败时回退已上传路径的基线，绝不把自己刚上传的内容再下载回来覆盖。
- **冲突保持「待解决」**：`metadata/conflicts.json`（设备本地、不进同步）记录待解决路径，本地保持为主文件、不自动覆盖，由用户显式解决；解决后刷新基线 + 远端清单，不会复发。
