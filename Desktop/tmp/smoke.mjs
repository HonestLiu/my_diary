...
var BrowserAdapter = class {
  ...
  async readText(path) {
    ...
    if (rec.text !== void 0) return rec.text;
    if (rec.bytes) return new TextDecoder().decode(rec.bytes);
    throw new Error(`File not found: ${path}`);
  }
  async writeText(path, content) {
    const rec = { path, text: content, updated: Date.now() };
  ...
  async writeBytes(path, data) {
    const rec = { path, bytes: data, updated: Date.now() };
    await tx("readwrite", (s) => s.put(rec));
  }
  async exists(path) {
    const rec = await tx(
  ...
  async list(prefix) {
    ...
    return all.map((r) => r.path).filter((p) => p.startsWith(prefix)).sort();
  }
...
var TauriFsAdapter = class {
  ...
  async resolveUrl(relPath) {
    const bytes = await this.readBytes(relPath).catch(() => null);
    if (!bytes) return "";
    const mime = mimeFromPath2(relPath);
    return bytesToDataUrl(bytes, mime);
  }
};
function mimeFromPath2(path) {
  const ext = path.split(".").pop()?.toLowerCase() ?? "";
  switch (ext) {
    case "webp":
      return "image/webp";
  ...
  async search(query) {
    ...
    for (const entry of this.map.values()) {
      ...
      for (const qt of qTokens) {
        const c = countIn(titleT, qt) * 3 + countIn(tagsT, qt) * 2 + countIn(locT, qt) * 1 + countIn(bodyT, qt) * 1;
        if (c > 0) {
          matched += 1;
          score += c;
        }
...
// src/lib/utils.ts
function formatDateKey(d = /* @__PURE__ */ new Date()) {
...
function buildSampleEntries() {
  ...
  const mk = (offsetDays, title, mood, body, tags) => {
...
var JournalRepository = class {
  ...
  async restoreVersion(dateKey, version) {
...
function toIndexed(meta, body) {
  ...
  return {
    ...
    date: meta.date,
    title: meta.title,
    body: body.replace(/[#>*_`~\-]/g, " ").slice(0, 2e4),
    tags: meta.tags.join(" "),
    mood: meta.mood,
...
async function main() {
  ...
  const e2 = mk("2026-08-11", "\u5DE5\u4F5C\u7B14\u8BB0", "Today I finished the report about Tokyo trip planning.", ["work"]);
  ...
  ok(r2[0].date === "2026-08-10", "\u300C\u5929\u6C14\u300D\u7ED3\u679C\u4E2D\u4E1C\u4EAC\u65C5\u884C\u6392\u524D(\u6807\u9898\u52A0\u6743)");
  console.log("[4] search \u2014 \u82F1\u6587\u8BCD + \u4E2D\u6587\u8BCD");
  const r3 = await repo.search("tokyo");
  ok(r3.some((e) => e.date === "2026-08-11"), "\u641C\u300Ctokyo\u300D\u547D\u4E2D\u5DE5\u4F5C\u7B14\u8BB0(\u5927\u5C0F\u5199\u4E0D\u654F\u611F)");
  const r4 = await repo.search("\u62A5\u544A");
  ok(r4.some((e) => e.date === "2026-08-11"), "\u641C\u300C\u62A5\u544A\u300D\u547D\u4E2D\u5DE5\u4F5C\u7B14\u8BB0\u6B63\u6587");
