...
function requireLoader() {
  ...
  function rollbackAnchorTransaction(state) {
    ...
    for (let index = names.length - 1; index >= 0; index -= 1) {
      const entry2 = transaction[names[index]];
      if (entry2.existed) {
        state.anchorMap[names[index]] = entry2.value;
      } else {
...
function serializeEntryFile(entry2) {
  const fm = buildFrontmatter(entry2);
...
function normalizeMeta(obj) {
  ...
  return { id, date, title, mood, weather, location, tags, created_at, updated_at };
}
function buildFrontmatter(entry2) {
  return {
    id: entry2.id,
    date: entry2.date,
    title: entry2.title,
    mood: entry2.mood,
    weather: entry2.weather,
...
function conflictFilePath(dateKey, side) {
  return `conflicts/${dateKey}.${side}.md`;
}
function dateKeyFromEntryPath(path) {
  const m = /entries\/\d{4}\/\d{2}\/(\d{4}-\d{2}-\d{2})\.md$/.exec(path);
  return m?.[1] ?? null;
}

// scripts/phase2_test.ts
var failures = 0;
...
var entry = {
  ...
  body: "## \u65E9\u5B89\n\n\u8FD9\u662F\u4E00\u6BB5 **\u6B63\u6587**\uFF0C\u5305\u542B emoji \u{1F31E} \u548C\u6362\u884C\u3002\n\n- \u5217\u8868\u9879"
};
var md = serializeEntryFile(entry);
console.log(md);
var parsed = parseEntryFile(md);
...
assert(
  conflictFilePath("2026-08-11", "local") === "conflicts/2026-08-11.local.md",
  "conflictFilePath \u6B63\u786E"
);
assert(
